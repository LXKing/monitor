...
//点击树节点是为true的时候
function beforeClickistrue(treeNode,e) {
...
function datacompanyandcar(data,counlist) {
    ...
    form.on('select(Companypanelqucarlay)',function (data) {
        if(data.value=="0"){  $("#finishingTask").bootstrapTable("load", insides);}else {
            var listid = [];
            var listdata =[];
            for(var i = 0 ;i<insides.length;i++){
...
function textscresh() {
    ...
    $("#textscreshcaronline").on("keyup",function(event){
        ...
        if(Name==""||Name.length==0){  $("#finishingTaskonline").bootstrapTable("load", finishingTaskonlinedata);}else {
            var reg = new RegExp(Name);
            //全部
            var listid =[];
            var listdata =[];
    ...
    $("#textscreshcaroffline").on("keyup",function(event){
        ...
        if(Name==""||Name.length==0){  $("#finishingTaskoffline").bootstrapTable("load", finishingTaskofflinedata);}else {
            var reg = new RegExp(Name);
            //全部
            var listid =[];
            var listdata =[];
        ...
        if(Name==""||Name.length==0){
            ...
            for(var i =0;i<trcount.length;i++){
                if($(trcount[i].children[1]).text().trim().match(reg)){
                    ...
                    var Name = {showName:$(trcount[i].children[1]).text().trim(),id:arrlist[0],lng:arrlist[1],alt:arrlist[2]};
                    listid.push(Name);
                }
            }
