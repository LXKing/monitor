var Ren_vehicleAlarmCount = function (url) {
    ...
    table.render({
        ...
        cols: [
            [
                ...
                {field: 'fatigueDriving', title: '疲劳驾驶次数', sort: true},
                {field: 'overspeedInArea', title: '区域内超速次数', sort: true},
                {field: 'overspeedInSection', title: '路段超速次数', sort: true},
                {field: 'overspeedNoneArea', title: '非区域超速次数', sort: true},
                {field: 'parkingTimeout', title: '超时停车次数', sort: true},
                {field: 'routeDeparture', title: '路线偏离次数', sort: true},
        ...
        response: {
            ...
            var source = [];
            var p = ['product', '疲劳驾驶次数', '区域内超速次数', '路段超速次数', '非区域超速次数', '超时停车次数', '路线偏离次数'];
            source[0] = p;
            for (let it of res.rows) {
                let arr = [];
                arr.push(it.motorcade);
                arr.push(it.fatigueDriving);
                arr.push(it.overspeedInArea);
                arr.push(it.overspeedInSection);
                arr.push(it.overspeedNoneArea);
                arr.push(it.parkingTimeout);
                arr.push(it.routeDeparture);
        ...
        if (obj.event === 'more') {
            ...
            var jjj = layer.open({
                ...
                , yes: function () {
                    ...
                    jQuery.ajax({
                        url: '/statistics/vehicleAlarmCountExport',
                        type: "POST",
                        data: params,
                        success: function (response, status, request) {
                            var url='/statistics/vehicleAlarmCountExport';
                ...
                success: function (layero, index) {
                    ...
                    table.render({
                        elem: '#detail'
                        , url: '/statistics/vehicleAlarmDetail', //数据接口
                        page: true, // 开启分页
                        method: "POST",
                        where: {
                            motorcade: data.motorcade,
