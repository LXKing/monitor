var Ren_historyOnlineTimeCount = function (url) {
    ...
    table.render({
        ...
        cols: [
            [
                ...
                {field: 'must', title: '应在线时长', sort: true},
                {field: 'real', title: '实在线时长', sort: true},
                {field: 'onlineRate', title: '在线率', sort: true},
                {field: 'offlineRate', title: '离线率', sort: true},
        ...
        response: {
            ...
            var source = [];
            var p = ['product', '应在线时长', '实在线时长'];
            source[0] = p;
            for (let it of res.rows) {
                let arr = [];
                arr.push(it.motorcade);
                arr.push(it.must);
                arr.push(it.offlrealine);
        ...
        if (obj.event === 'more') {
            ...
            var jjj = layer.open({
                ...
                , yes: function () {
                    ...
                    jQuery.ajax({
                        url: '/statistics/historyOnlineTimeCountExport',
                        type: "POST",
                        data: params,
                        success: function (response, status, request) {
                            var url='/statistics/historyOnlineTimeCountExport';
                ...
                success: function (layero, index) {
                    ...
                    table.render({
                        elem: '#detail'
                        , url: '/statistics/historyOnlineTimeDetail', //数据接口
                        page: true, // 开启分页
                        method: "POST",
                        where: {
                            motorcade: data.motorcade,
