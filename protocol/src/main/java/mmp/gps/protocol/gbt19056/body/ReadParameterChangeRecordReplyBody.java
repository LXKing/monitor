...
public class ReadParameterChangeRecordReplyBody implements IPacket {
    ...
    public void from(byte[] src) {
        ...
        while (io.hasRemaining()) {
            ...
            Short action = Short.valueOf(io.getUbyte());
            this.logs.add(Tuple.of(time, action));
        }
    ...
    public byte[] array() {
        if (this.logs != null && this.logs.size() > 0) {
            ...
            while (var3.hasNext()) {
                ...
                io.putUbyte(((Short) tuple.t).shortValue());
            }
