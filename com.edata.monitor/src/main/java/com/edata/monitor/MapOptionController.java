...
public class MapOptionController {
	...
	public Object query(HttpServletRequest request) throws Exception {
		...
		return userService.getUserOptions(identify.getId(), UserOptions.MapOption);
	}

	@RequestMapping(value = "/mapOption/save", method = RequestMethod.POST)
	public String save(@RequestParam double lat, @RequestParam double lng, @RequestParam int zoom, HttpServletRequest request, RedirectAttributes r) {
		...
		try {
			...
			userService.saveUserOptions(identify.getId(), UserOptions.MapOption, values);
			WebUtil.success(r);
		} catch (Exception ex) {
