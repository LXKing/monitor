...
function treeHeader(cols) {
    ...
    for (k in cols) {
        ...
        header += `<div class="tabular_col  ${cols[k].key}"> ${cols[k].name} </div> `;
    }
    header += '</a></li>';
...
function diyDom(treeId, treeNode, tSetting) {
    ...
    var kkk = `<div class="tabular_col  ${tSetting.cols[0].key}">   </div> `;
    ...
    for (k in tSetting.cols) {
        ...
        var ss=   treeNode[tSetting.cols[k].key] ;
        var kkk = `<div class="tabular_col  ${tSetting.cols[k].key}"> ${getData(ss)} </div> `;
        editStr += kkk;
