...
import mmp.gps.domain.feature.FeatureDto;
import mmp.gps.domain.feature.FeatureInfo;
import mmp.gps.domain.feature.MatchFeatureRequest;
import mmp.gps.domain.feature.MatchFeatureResponse;
...
import mmp.gps.domain.log.LocateLogDto;
import mmp.gps.domain.log.SpeedLogDto;
import mmp.gps.domain.log.SpeedStatusLogDto;
import mmp.gps.domain.log.*;
import mmp.gps.domain.multimedia.*;
import mmp.gps.domain.parameter.LoadParameterInfo;
import mmp.gps.domain.parameter.LoadParametersRequest;
import mmp.gps.domain.parameter.LoadParametersResponse;
import mmp.gps.domain.parameter.ParameterDto;
...
public class GodpDao implements IGodpDao {
    ...
    public Map<String, LatestDto> loadLatest(List<String> numbers) throws Exception {
        ...
        if (data == null)
            return result;
        for (Map.Entry<String, LatestInfo> entry : data.entrySet()) {
    ...
    public List<FeatureDto> loadFeatures(String number) throws Exception {
        ...
        Device device = deviceDao.getByNum(number);


        request.setProtocol(Integer.parseInt(device.getProtocol().substring(2), 16));
