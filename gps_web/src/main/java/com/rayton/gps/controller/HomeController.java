...
public class HomeController {
    ...
    public Object login(@ModelAttribute("login") @Valid Login model, BindingResult bindings, HttpSession session, HttpServletResponse response) throws Exception {
        ...
        if (!subject.isAuthenticated()) {
            ...
            // byte[] aa = StupidRSA.encryptByPublicKey("888888", map.get("publicKey"));
            // System.out.println(new String(aa));
            String encodedData = model.getPwd();
            // System.out.println(encodedData.getBytes() == aa);
            // byte[] decodedData = null;
            // try {
            //     // String s = map.get("privateKey");
            //     //
            //     // decodedData = StupidRSA.decryptByPrivateKey(encodedData.getBytes(), s);
            //     // decodedData = StupidRSA.decryptByPrivateKey(aa, s);
            // } catch (Exception e) {
            //     e.printStackTrace();
            // }
    ...
    @ResponseBody
    public Object myinfo(Model model, HttpServletRequest request) throws Exception {
    ...
    @ResponseBody
    public ResponseEntity<Map<Object, Object>> myinfo(@ModelAttribute("myinfo") @Valid MyInfo info, BindingResult binding) throws Exception {
        ...
        IdentityDto user = (IdentityDto) SecurityUtils.getSubject().getPrincipals().getPrimaryPrincipal();
        // return "/myinfo.form";
        securityService.saveMyinfo(info, user);
    ...
    @ResponseBody
    public ResponseEntity<Map<Object, Object>> mykey(@ModelAttribute("mykey") @Valid SaveMyKeyRequest mykey) throws Exception {
        IdentityDto user = (IdentityDto) SecurityUtils.getSubject().getPrincipals().getPrimaryPrincipal();
        // Identity user = (Identity) request.getAttribute("user");
        // securityService.saveMyKey(user.getId(), mykey.getOldKey(), mykey.getNewKey(), mykey.getConfirmKey());
        securityService.saveMyKey(user.getId(), user.getAccount(), mykey.getOldKey(), mykey.getNewKey(), mykey.getConfirmKey());
