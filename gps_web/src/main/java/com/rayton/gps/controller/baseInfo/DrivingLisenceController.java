...
public class DrivingLisenceController {
    ...
    public Object query(@RequestParam(required = false) String filter, @RequestParam int page, @RequestParam int limit) throws Exception {
        ...
        assist.setRowSize(limit).setStartRow((page - 1) * limit);

        if (StringUtils.isNotEmpty(filter)) {
            assist.setRequires(Assist.andLike("name", filter));
    ...
    @ResponseBody
    public ResponseEntity<Map<Object, Object>> create(@ModelAttribute("drivingLicence") @Valid DrivingLisence drivingLicence, BindingResult binding) throws Exception {
        IdentityDto identity = (IdentityDto) SecurityUtils.getSubject().getPrincipals().getPrimaryPrincipal();
    ...
    @ResponseBody
    public ResponseEntity<Map<Object, Object>> edit(@ModelAttribute("drivingLicence") @Valid DrivingLisence drivingLicence, BindingResult binding) throws Exception {
        if (binding.hasErrors())
            return ResponseEntityWrapper.Failed();
    ...
    @ResponseBody
    public ResponseEntity<Map<Object, Object>> delete(@RequestParam Integer id) throws Exception {
