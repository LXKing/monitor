...
import mmp.gps.domain.instruct.DeviceInstruct;
import mmp.gps.monitor.service.SyncInstructService;
...
public class SynchronizerCache {
    private static final Logger LOGGER = LoggerFactory.getLogger(SynchronizerCache.class);

    private static SyncInstructService syncInstructService;
    private static Map<String, DeviceInstructSender> senders = new ConcurrentHashMap<>();
    ...
    public SynchronizerCache(@Qualifier("syncInstructService") SyncInstructService syncInstructService) {
        ...
        areas.forEach(deviceInAreaInfo -> {
            DeviceInstructSender sender = SynchronizerCache.senders.get(deviceInAreaInfo.getDeviceNumber());
            if (sender == null) {
                sender = new DeviceInstructSender();
                sender.setDeviceNumber(deviceInAreaInfo.getDeviceNumber());
                sender.setSent(new Date());
                SynchronizerCache.senders.put(deviceInAreaInfo.getDeviceNumber(), sender);
            }

            SynchronizerCache.put(deviceInAreaInfo);
        });
    ...
    private static void routeAreaSync(DeviceInAreaInfo info) {
        if (info.getAction() == AreaActions.Remove.getIndex()) {
            ...
            //     senders.put(info.getDeviceNumber(), sender);
            // }
            updateSenderIfNull(sender, info);
    ...
    private static void polygonAreaSync(DeviceInAreaInfo info) {
        if (info.getAction() == AreaActions.Remove.getIndex()) {
            ...
            //     senders.put(info.getDeviceNumber(), sender);
            // }
            sender = updateSenderIfNull(sender, info);
            DeviceInstruct deviceInstruct = new DeviceInstruct();
            deviceInstruct.setSerialNumber(info.getId());
    ...
    private static DeviceInstructSender updateSenderIfNull(DeviceInstructSender sender, DeviceInAreaInfo info) {
